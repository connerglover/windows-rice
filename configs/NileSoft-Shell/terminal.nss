menu(type='*' where=(sel.count or wnd.is_taskbar or wnd.is_edit) title=title.terminal sep='top' image=icon.run_with_powershell)
{
	...
	item(title='WezTerm' tip=tip_run_admin admin=has_admin image='C:\Program Files\WezTerm\wezterm-gui.exe' cmd='C:\Program Files\WezTerm\wezterm-gui.exe' args='start --cwd "@sel.dir"')
}
